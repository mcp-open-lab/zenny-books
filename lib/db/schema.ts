...
export const documents = pgTable(
  ...
  (table) => [
    ...
    index("documents_import_batch_id_idx").on(table.importBatchId),
  ]
);
...
export const importBatches = pgTable(
  ...
  (table) => [
    index("import_batches_user_id_created_at_idx").on(
      table.userId,
      table.createdAt
    ),
...
export const importBatchItems = pgTable(
  ...
  (table) => [
    index("import_batch_items_batch_id_idx").on(table.batchId),
...
export const batchActivityLogs = pgTable(
  ...
  (table) => [
    index("batch_activity_logs_batch_id_idx").on(table.batchId),
    index("batch_activity_logs_batch_item_id_idx").on(table.batchItemId),
    index("batch_activity_logs_created_at_idx").on(table.createdAt),
  ]
);
...
export const receipts = pgTable(
  ...
  (table) => [
    index("receipts_user_id_date_idx").on(table.userId, table.date),
...
export const bankStatementTransactions = pgTable(
  ...
  (table) => [
    index("bank_statement_transactions_bank_statement_id_idx").on(
      table.bankStatementId
    ),
    index("bank_statement_transactions_transaction_date_idx").on(
...
export const llmLogs = pgTable(
  ...
  (table) => [
    index("llm_logs_user_id_idx").on(table.userId),
    index("llm_logs_entity_id_idx").on(table.entityId),
    index("llm_logs_provider_idx").on(table.provider),
    index("llm_logs_prompt_type_idx").on(table.promptType),
